...
def sample_posterior_nuts(
    ...
    init_positions = jnp.stack([init_theta + 0.1 * random.normal(k, shape=(p,)) for k in init_keys])
    ...
    return {"theta_samples": theta_samples, "w_samples": w_samples, "diagnostics": diagnostics}
...
def compute_rhat(samples):
    ...
    B = num_samples / (num_chains - 1) * jnp.sum((chain_means - global_mean[None, :]) ** 2, axis=0)
...
def compute_ess(samples):
    ...
    acf1 = jnp.array([jnp.corrcoef(w_flat[:-1, i], w_flat[1:, i])[0, 1] for i in range(p)])
